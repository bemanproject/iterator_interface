...
template <typename T>
using use_base = decltype(iterator_interface_access::base(std::declval<T&>()));
...
BEMAN_ITERATOR_INTERFACE_DETAIL_STL_INTERFACES_NAMESPACE_V1 {
    ...
    struct iterator_interface {
      ...
      public:
        ...
        constexpr auto operator*() noexcept(noexcept(*iterator_interface_access::base(std::declval<D&>())))
            -> decltype(*iterator_interface_access::base(std::declval<D&>())) {
            return *iterator_interface_access::base(derived());
        }
        template <typename D = Derived>
        constexpr auto operator*() const noexcept(noexcept(*iterator_interface_access::base(std::declval<const D&>())))
        ...
        constexpr auto operator++() noexcept(noexcept(++iterator_interface_access::base(std::declval<D&>())))
            -> decltype(++iterator_interface_access::base(std::declval<D&>()), std::declval<D&>()) {
    ...
    constexpr auto operator==(IteratorInterface1 lhs, IteratorInterface2 rhs) noexcept
        -> decltype(iterator_interface_access::base(std::declval<IteratorInterface1&>()) ==
                    iterator_interface_access::base(std::declval<IteratorInterface2&>())) {
        return iterator_interface_access::base(lhs) == iterator_interface_access::base(rhs);
    }
...
BEMAN_ITERATOR_INTERFACE_DETAIL_STL_INTERFACES_NAMESPACE_V2 {
    ...
    // clang-format off
        concept base_3way =
            ...
            requires (D d, D2 d2) { iterator_interface_access::base(d) <=> iterator_interface_access::base(d2); };
#else
            false;
#endif
    // clang-format on
    ...
    public:
      ...
      constexpr decltype(auto) operator*() const
        requires requires (D const d) { *iterator_interface_access::base(d); } {
          return *iterator_interface_access::base(derived());
        }
    ...
    template<typename D1, typename D2>
      constexpr auto operator-(D1 lhs, D2 rhs)
        requires v2_dtl::derived_iter<D1> && v2_dtl::derived_iter<D2> &&
                 requires { iterator_interface_access::base(lhs) - iterator_interface_access::base(rhs); }
          { return iterator_interface_access::base(lhs) - iterator_interface_access::base(rhs); }
    template<typename D>
      constexpr auto operator-(D it, typename D::difference_type n)
        requires v2_dtl::derived_iter<D> && requires { it += -n; }
